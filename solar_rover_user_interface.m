function varargout = deneme44_export(varargin)
% DENEME44_EXPORT MATLAB code for deneme44_export.fig
%      DENEME44_EXPORT, by itself, creates a new DENEME44_EXPORT or raises the existing
%      singleton*.
%
%      H = DENEME44_EXPORT returns the handle to a new DENEME44_EXPORT or the handle to
%      the existing singleton*.
%
%      DENEME44_EXPORT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DENEME44_EXPORT.M with the given input arguments.
%
%      DENEME44_EXPORT('Property','Value',...) creates a new DENEME44_EXPORT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before deneme44_export_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to deneme44_export_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help deneme44_export

% Last Modified by GUIDE v2.5 16-Apr-2023 22:28:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @deneme44_export_OpeningFcn, ...
                   'gui_OutputFcn',  @deneme44_export_OutputFcn, ...
                   'gui_LayoutFcn',  @deneme44_export_LayoutFcn, ...
                   'gui_Callback',   []);
...
% --- Executes just before deneme44_export is made visible.
function deneme44_export_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to deneme44_export (see VARARGIN)

% Choose default command line output for deneme44_export
handles.output = hObject;

% create an axes that spans the whole gui
ah = axes('unit', 'normalized', 'position', [0 0 1 1]); 
% import the background image and show it on the axes
bg = imread('pattern-369543.png'); imagesc(bg);
...
% UIWAIT makes deneme44_export wait for user response (see UIRESUME)
...
function varargout = deneme44_export_OutputFcn(hObject, eventdata, handles) 
...
while(x<50)
    x = x+1;
    y1(x) = fscanf(comport,'%f'); %receive temp;
    y2(x) = fscanf(comport,'%f');
    ...
    plot(handles.axes1,y1,'r','LineWidth',2);
    title(handles.axes1,'temp');
    axis(handles.axes1,[0 50 0 30]);
    xlabel(handles.axes1,'Time in secons');
    ylabel(handles.axes1,'Digital Value');
    grid 
    ...
    plot(handles.axes2,y2,'b','LineWidth',2);
    title(handles.axes2,'humidity');
    axis(handles.axes2,[0 50 0 80]);
    xlabel(handles.axes2,'Time in secons');
    ylabel(handles.axes2,'Digital Value');
    set(handles.edit3,'String',num2str(hum));
    grid 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% cam = ipcam('http://192.168.1.35:8080/videofeed');
global  cam i;
i=1;
ip=get(handles.IP,'String');
cam= ipcam(ip);
himage = image(zeros(360,640,3),'parent',handles.axes3);
preview(cam,himage);
set(handles.state2,'String','Connect');
set(handles.state2,'ForegroundColor','g');
...
global  cam i;
i=0;
set(handles.state2,'String','Disconnect');
set(handles.state2,'ForegroundColor','r');
closePreview(cam);
cla(handles.axes3)
...
function h1 = deneme44_export_LayoutFcn(policy)
...
appdata.GUIDEOptions = struct(...
    ...
    'syscolorfig', 1, ...
    'blocking', 0, ...
    'lastSavedFile', 'D:\deneme44_export.m', ...
    'lastFilename', 'D:\masaüstü önemli dosyalar\projeler2\arduino\vers1.0 matlab\deneme44.fig');
...
'PaperUnits','inches',...
'Units','characters',...
'Position',[135.8 29.1493212669683 260.428571428571 55.2352941176471],...
...
'PlotBoxAspectRatio',[1 0.613636363636364 0.613636363636364],...
...
'Position',[3.28571428571429 41.5882352941176 44 11.1176470588235],...
'InnerPosition',[3.28571428571429 41.5882352941176 44 11.1176470588235],...
...
'LooseInset',[14.56 3.55882352941176 10.64 2.42647058823529],...
...
'Color',[0 0 0],...
'ColorMode','auto',...
'Position',[0.500000551149443 1.01309523809524 0.500000000000007],...
'PositionMode','auto',...
'Interpreter','tex',...
...
'ColorMode','auto',...
'Position',[0.500000476837158 -0.134920637443583 7.105427357601e-15],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',
...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[0.500000476837158 -0.134210528825458 0],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[-0.0920634936529494 0.500000476837159 0],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',90,...
...
'ZRulerMode',get(0,'defaultaxesZRulerMode'),...
'AmbientLightSourceMode',get(0,'defaultaxesAmbientLightSourceMode'),...
'Position',[105.428571428571 41.1176470588235 45 11.1764705882353],...
'InnerPosition',[105.428571428571 41.1176470588235 45 11.1764705882353],...
'ActivePositionProperty','position',...
'ActivePositionPropertyMode',get(0,'defaultaxesActivePositionPropertyMode'),...
'PositionConstraint','innerposition',...
'PositionConstraintMode',get(0,'defaultaxesPositionConstraintMode'),...
...
'Color',[0 0 0],...
'ColorMode','auto',...
'Position',[0.50000271418738 1.01302631578947 0.5],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[-0.0920634936529496 0.500000476837159 0],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',90,...
...
'ZRulerMode',get(0,'defaultaxesZRulerMode'),...
'AmbientLightSourceMode',get(0,'defaultaxesAmbientLightSourceMode'),...
'Position',[159.285714285714 43.7058823529412 45 11.1764705882353],...
'InnerPosition',[159.285714285714 43.7058823529412 45 11.1764705882353],...
'ActivePositionProperty','position',...
'ActivePositionPropertyMode',get(0,'defaultaxesActivePositionPropertyMode'),...
'PositionConstraint','innerposition',...
'PositionConstraintMode',get(0,'defaultaxesPositionConstraintMode'),...
...
'Color',[0 0 0],...
'ColorMode','auto',...
'Position',[0.500003855569023 1.01302631578947 0.5],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[0.500000476837159 -0.134210528825459 0],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[-0.0920634936529492 0.500000476837158 0],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',90,...
...
'ZRulerMode',get(0,'defaultaxesZRulerMode'),...
'AmbientLightSourceMode',get(0,'defaultaxesAmbientLightSourceMode'),...
'Position',[212.714285714286 43.7058823529412 45 11.1764705882353],...
'InnerPosition',[212.714285714286 43.7058823529412 45 11.1764705882353],...
'ActivePositionProperty','position',...
'ActivePositionPropertyMode',get(0,'defaultaxesActivePositionPropertyMode'),...
'PositionConstraint','innerposition',...
'PositionConstraintMode',get(0,'defaultaxesPositionConstraintMode'),...
...
'Color',[0 0 0],...
'ColorMode','auto',...
'Position',[0.500004987868051 1.01302631578947 0.5],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[0.500000476837158 -0.134210528825459 0],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[-0.0920634936529501 0.500000476837158 0],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',90,...
...
'String','Start Real Time',...
'Style',get(0,'defaultuicontrolStyle'),...
'Position',[-0.142857142857143 33.2352941176471 16 3.70588235294117],...
'BackgroundColor',[0.901960784313726 0.901960784313726 0.901960784313726],...
'Callback',@(hObject,eventdata)deneme44_export('pushbutton1_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[0.392156862745098 0.831372549019608 0.0745098039215686],...
'Tag','pushbutton1',...
'FontWeight','bold',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'String','Temperature',...
'Style','text',...
'Position',[-0.142857142857143 37.4705882352941 13.7142857142857 1.35294117647059],...
'Children',[],...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'Tag','text2',...
'FontSize',10);
...
'String','Temperature',...
'Style','edit',...
'Position',[1.025 5.28333333333333 0.791666666666667 0.225],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('edit1_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[1 0 0],...
'HandleVisibility',get(0,'defaultuicontrolHandleVisibility'),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('edit1_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'Tag','edit1',...
'KeyPressFcn',@(hObject,eventdata)deneme44_export('edit1_KeyPressFcn',hObject,eventdata,guidata(hObject)),...
'KeyReleaseFcn','%automatic');
...
'Units','characters',...
'FontUnits',get(0,'defaultuicontrolFontUnits'),...
'String','Control IP Camera',...
'Style','text',...
'Position',[190 36.9411764705882 42.7142857142857 2.52941176470588],...
'Children',[],...
'ForegroundColor',[1 0.411764705882353 0.16078431372549],...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'Tag','text3',...
'FontSize',20,...
...
'Tag','uipanel3',...
'UserData',[],...
'Position',[159.285714285714 2.52941176470588 99.4285714285714 7],...
'Layout',[],...
'FontSize',10,...
...
'Position',[1.28571428571429 3.29411764705882 11.5714285714286 2.23529411764706],...
'BackgroundColor',[1 1 1],...
'Callback',blanks(0),...
'Children',[],...
'Tooltip',blanks(0),...
...
appdata = [];
appdata.lastValidTag = 'IP';

h33 = uicontrol(...
'Parent',h31,...
'Units','characters',...
'FontUnits',get(0,'defaultuicontrolFontUnits'),...
'String','Write IP Adress URL',...
'Style','edit',...
'Position',[15.4285714285714 3.58823529411765 80 1.52941176470588],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('IP_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('IP_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'Tag','IP');
...
'String','Connect',...
'Style',get(0,'defaultuicontrolStyle'),...
'Position',[34.2857142857143 1.64705882352941 18.1428571428571 1.64705882352941],...
'Callback',@(hObject,eventdata)deneme44_export('connectbutton_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[0.466666666666667 0.674509803921569 0.188235294117647],...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'Tag','connectbutton',...
'FontSize',15);

appdata = [];
appdata.lastValidTag = 'disconnectbutton';
...
'String','Disconnect',...
'Style',get(0,'defaultuicontrolStyle'),...
'Position',[57.2857142857143 1.58823529411765 18.1428571428572 1.64705882352941],...
'Callback',@(hObject,eventdata)deneme44_export('disconnectbutton_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[1 0 0],...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
...
'String','State : ',...
'Style','text',...
'Position',[1.85714285714286 0.647058823529412 10.1428571428571 0.941176470588235],...
'Children',[],...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'Tag','text5');

appdata = [];
appdata.lastValidTag = 'state2';
...
'String','Disconnect',...
'Style','text',...
'Position',[10.1428571428571 0.588235294117647 9.71428571428571 0.941176470588235],...
'Children',[],...
'ForegroundColor',[1 0 0],...
'Tag','state2',...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'String','Humidity',...
'Style','text',...
'Position',[53 37.1176470588235 9.42857142857143 1.35294117647059],...
'Children',[],...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'DeleteFcn',blanks(0),...
...
'String','Humidity',...
'Style','edit',...
'Position',[4.125 5.225 0.791666666666667 0.225],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('edit3_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[0.301960784313725 0.745098039215686 0.933333333333333],...
'HandleVisibility',get(0,'defaultuicontrolHandleVisibility'),...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('edit3_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'DeleteFcn',blanks(0),...
'Tag','edit3',...
'KeyPressFcn',blanks(0));
...
'String','Pressure',...
'Style','text',...
'Position',[106.142857142857 37.8823529411765 9.85714285714286 1.35294117647059],...
'Children',[],...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'DeleteFcn',blanks(0),...
...
'String','Pressure',...
'Style','edit',...
'Position',[7.225 5.33333333333333 0.791666666666666 0.225],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('edit4_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[0.635294117647059 0.0784313725490196 0.184313725490196],...
'HandleVisibility',get(0,'defaultuicontrolHandleVisibility'),...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('edit4_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'DeleteFcn',blanks(0),...
'Tag','edit4',...
'KeyPressFcn',blanks(0));
...
'Style','text',...
'Value',get(0,'defaultuicontrolValue'),...
'Position',[159.285714285714 40.7647058823529 9.42857142857142 1.35294117647059],...
...
'Style','edit',...
'Value',get(0,'defaultuicontrolValue'),...
'Position',[10.325 5.74166666666667 0.791666666666666 0.225],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('edit5_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'Tooltip',blanks(0),...
'ForegroundColor',[1 1 0.0666666666666667],...
'Enable',get(0,'defaultuicontrolEnable'),...
'Visible',get(0,'defaultuicontrolVisible'),...
'HandleVisibility',get(0,'defaultuicontrolHandleVisibility'),...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('edit5_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'DeleteFcn',blanks(0),...
'Tag','edit5',...
'UserData',[],...
'KeyPressFcn',blanks(0),...
...
'String','Velocity',...
'Style','text',...
'Position',[212.428571428571 40.8235294117647 8.57142857142858 1.35294117647059],...
'Children',[],...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'DeleteFcn',blanks(0),...
...
'String','Humidity',...
'Style','edit',...
'Position',[13.425 5.75 0.791666666666666 0.225],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('edit6_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[1 0.0745098039215686 0.650980392156863],...
'HandleVisibility',get(0,'defaultuicontrolHandleVisibility'),...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('edit6_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'DeleteFcn',blanks(0),...
'Tag','edit6',...
'KeyPressFcn',blanks(0));
...
'FontUnits','points',...
'Units','characters',...
'ForegroundColor',[0.0745098039215686 0.623529411764706 1],...
'Title','Monitor',...
'Tag','uibuttongroup3',...
'Position',[167.142857142857 10.3529411764706 86.8571428571429 25.1764705882353],...
'Layout',[],...
'FontSize',15,...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'CameraViewAngle',6.60861036031192,...
'CameraViewAngleMode',get(0,'defaultaxesCameraViewAngleMode'),...
'PlotBoxAspectRatio',[1 0.776548672566372 0.776548672566372],...
'PlotBoxAspectRatioMode',get(0,'defaultaxesPlotBoxAspectRatioMode'),...
'Colormap',[0.2422 0.1504 0.6603;0.2444 0.1534 0.6728;0.2464 0.1569 0.6847;0.2484 0.1607 0.6961;0.2503 0.1648 0.7071;0.2522 0.1689 0.7179;0.254 0.1732 0.7286;0.2558 0.1773 0.7393;0.2576 0.1814 0.7501;0.2594 0.1854 0.761;0.2611 0.1893 0.7719;0.2628 0.1932 0.7828;0.2645 0.1972 0.7937;0.2661 0.2011 0.8043;0.2676 0.2052 0.8148;0.2691 0.2094 0.8249;0.2704 0.2138 0.8346;0.2717 0.2184 0.8439;0.2729 0.2231 0.8528;0.274 0.228 0.8612;0.2749 0.233 0.8692;0.2758 0.2382 0.8767;0.2766 0.2435 0.884;0.2774 0.2489 0.8908;0.2781 0.2543 0.8973;0.2788 0.2598 0.9035;0.2794 0.2653 0.9094;0.2798 0.2708 0.915;0.2802 0.2764 0.9204;0.2806 0.2819 0.9255;0.2809 0.2875 0.9305;0.2811 0.293 0.9352;0.2813 0.2985 0.9397;0.2814 0.304 0.9441;0.2814 0.3095 0.9483;0.2813 0.315 0.9524;0.2811 0.3204 0.9563;0.2809 0.3259 0.96;0.2807 0.3313 0.9636;0.2803 0.3367 0.967;0.2798 0.3421 0.9702;0.2791 0.3475 0.9733;0.2784 0.3529 0.9763;0.2776 0.3583 0.9791;0.2766 0.3638 0.9817;0.2754 0.3693 0.984;0.2741 0.3748 0.9862;0.2726 0.3804 0.9881;0.271 0.386 0.9898;0.2691 0.3916 0.9912;0.267 0.3973 0.9924;0.2647 0.403 0.9935;0.2621 0.4088 0.9946;0.2591 0.4145 0.9955;0.2556 0.4203 0.9965;0.2517 0.4261 0.9974;0.2473 0.4319 0.9983;0.2424 0.4378 0.9991;0.2369 0.4437 0.9996;0.2311 0.4497 0.9995;0.225 0.4559 0.9985;0.2189 0.462 0.9968;0.2128 0.4682 0.9948;0.2066 0.4743 0.9926;0.2006 0.4803 0.9906;0
...
'ZRulerMode',get(0,'defaultaxesZRulerMode'),...
'AmbientLightSourceMode',get(0,'defaultaxesAmbientLightSourceMode'),...
'Position',[9.71428571428571 1.94117647058824 64.5714285714286 20.6470588235294],...
'InnerPosition',[9.71428571428571 1.94117647058824 64.5714285714286 20.6470588235294],...
'ActivePositionProperty','position',...
'ActivePositionPropertyMode',get(0,'defaultaxesActivePositionPropertyMode'),...
'PositionConstraint','innerposition',...
'PositionConstraintMode',get(0,'defaultaxesPositionConstraintMode'),...
'LooseInset',[33.7814285714286 5.58411764705882 24.6864285714286 3.80735294117647],...
'LooseInsetMode',get(0,'defaultaxesLooseInsetMode'),...
'ColorOrder',get(0,'defaultaxesColorOrder'),...
'SortMethod','childorder',...
'SortMethodMode',get(0,'defaultaxesSortMethodMode'),...
...
'Color',[0 0 0],...
'ColorMode','auto',...
'Position',[0.500000622420185 1.00783475783476 0.500000000000007],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[0.500000476837158 -0.0835707516638529 7.105427357601e-15],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',0,...
...
'Color',[0.15 0.15 0.15],...
'ColorMode','auto',...
'Position',[-0.0789085558382108 0.500000476837158 7.105427357601e-15],...
'PositionMode','auto',...
'Interpreter','tex',...
'InterpreterMode','auto',...
'Rotation',90,...
...
'String',{  'Slider' },...
'Style','slider',...
'Position',[125.571428571429 12.0588235294118 41.1428571428572 2],...
'BackgroundColor',[0.9 0.9 0.9],...
'Callback',@(hObject,eventdata)deneme44_export('slider1_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('slider1_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'Tag','slider1');
...
'String','Camera Angle',...
'Style','text',...
'Position',[127.857142857143 9.35294117647059 15.5714285714286 1.35294117647059],...
'Children',[],...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'DeleteFcn',blanks(0),...
...
'String','Angle',...
'Style','edit',...
'Position',[8.49166666666667 1.29166666666667 0.791666666666666 0.225],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('edit7_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',get(0,'defaultuicontrolForegroundColor'),...
'HandleVisibility',get(0,'defaultuicontrolHandleVisibility'),...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('edit7_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'DeleteFcn',blanks(0),...
'Tag','edit7',...
'KeyPressFcn',blanks(0));
...
'Style','text',...
'Position',[-0.142857142857143 26 17.4285714285714 2.29411764705882],...
'Children',[],...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} ,...
'DeleteFcn',blanks(0),...
...
'String','Percentage',...
'Style','edit',...
'Position',[1.025 3.78333333333333 0.791666666666667 0.225],...
'BackgroundColor',[1 1 1],...
'Callback',@(hObject,eventdata)deneme44_export('edit8_Callback',hObject,eventdata,guidata(hObject)),...
'Children',[],...
'ForegroundColor',[0.929411764705882 0.694117647058824 0.125490196078431],...
'HandleVisibility',get(0,'defaultuicontrolHandleVisibility'),...
'ButtonDownFcn',blanks(0),...
'CreateFcn', {@local_CreateFcn, @(hObject,eventdata)deneme44_export('edit8_CreateFcn',hObject,eventdata,guidata(hObject)), appdata} ,...
'DeleteFcn',blanks(0),...
'Tag','edit8',...
'KeyPressFcn',blanks(0));
...
elseif local_isInvokeHGCallback(gui_State, varargin{:})
    % DENEME44_EXPORT('CALLBACK',hObject,eventData,handles,...)
    gui_Create = false;
else
    % DENEME44_EXPORT(...)
